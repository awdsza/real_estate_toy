...
export default function SearchHeader() {
  const { searchMode, changeSearchMode, path } = useCommonContext();
  const toggleSearchMode = (mode) => {
    changeSearchMode(mode);
  };

  return (
    <header className="flex flex-col w-full sticky top-0 left-0 right-0 z-10 bg-mainColor">
      <div className="w-full font-bold text-2xl text-baseColor">
        <Link to="/">
